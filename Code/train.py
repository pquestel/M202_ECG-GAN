...
def GetECGData(source_file):
  ...
  """
Creating the training set of ecg signals
"""

source_filename =  r'C:\Users\phili\OneDrive\Desktop\Electrical Engineering MS - UCLA\202 Embedded Systems\Project - ECG Generator\Test Sine\ECG\trainData.csv'
...
test_filename =  r'C:\Users\phili\OneDrive\Desktop\Electrical Engineering MS - UCLA\202 Embedded Systems\Project - ECG Generator\Test Sine\ECG\testData.csv'

ecg_data_test = GetECGData(source_file = test_filename)
      ...
      if n_batch ==( num_batches - 1):
        ...
        # Check how the generator is doing by saving G's output on fixed_noise
          ...
          with torch.no_grad():
              ...
              testloader=torch.utils.data.DataLoader(ecg_data_test, batch_size=sample_size, shuffle=True)
              ...
              sigma = [pairwisedistances(ecg_data_test[:].type(torch.DoubleTensor),generated_sample.type(torch.DoubleTensor).squeeze())] 
              mmd = MMDStatistic(len(ecg_data_test[:]),generated_sample.size(0))
              mmd_eval = mmd(ecg_data_test[:].type(torch.DoubleTensor),generated_sample.type(torch.DoubleTensor).squeeze(),sigma, ret_matrix=False)
